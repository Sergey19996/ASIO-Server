...
{
    {
        ...
        for (auto const& [id, type] : minionMap->GetSquad()) {
            ...
            if (companion && !companion->IsDead()) {
                // Теперь у нас есть доступ к специфичным методам компаньона

                

                if (companion->GetCurrentState() == AIState::MoveToPoint) {
                    companion->SetState(AIState::Follow); // Сброс в дефолт
                }
                else {
                    companion->SetTemporaryGoal(targetPos, true);
                }
