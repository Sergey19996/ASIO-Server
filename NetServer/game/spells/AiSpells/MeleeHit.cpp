...
bool MeleeHit::Cast(Character& caster, Match* server)
{
...
void MeleeHit::ForceFinish(int uid, Match* server)
{
    //  parts.clear();  // не надо пытаться удалять уже удалённые projectiles
    state = SpellState::Finished;
}

bool MeleeHit::OwnsProjectile(int uid)
{
    return projectileId == (uint16_t)uid;
}

void MeleeHit::UpdateAppear(float dt, Match* server)
{
...
void MeleeHit::UpdateActive(float dt, Match* server)
{
    auto& entities = server->GetEntities();
    // for (auto& p : parts) {
    if (casterId >= entities.size() || !entities[casterId].active) return;
...
void MeleeHit::UpdateDisappear(float dt, Match* server)
{
