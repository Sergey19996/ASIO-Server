#include "StygianSpike.h"
#include "../NetShared/entities/Mage.h"
#include "../../../Match.h"
bool StygianSpike::Cast(Character& caster, Match* server)
{
    ...
    // Создаем снаряд
    projectileUid = server->CreateProjectile(ProjectileType::StygianSpike, p, basePos, direction, targetRadius);
...
bool StygianSpike::OwnsProjectile(int uid)
{
    return projectileUid == (uint16_t)uid;
}

void StygianSpike::ForceFinish(int uid, Match* server)
{
    ReleaseCaster(server);
...
void StygianSpike::UpdateAppear(float dt, Match* server)
{
    ...
    float chargeRatio = 1.0f - (lifeTimer / appearDuration);
    slot.data.fRadius = 0.25f + (chargeRatio * 0.6f); // От 0.85 до 0.25
...
void StygianSpike::UpdateActive(float dt, Match* server)
{
...
void StygianSpike::UpdateDisappear(float dt, Match* server)
{
